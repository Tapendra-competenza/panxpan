<template>
  <div class="containerTest">
    <img  v-bind:src="marketingImage"  style="width:100%" id="bgImage">
      <div class="center">
        <img class="countryLogo" v-bind:src="countryLogo">
      </div>
      <div class="center">
        <p class="textCenter">
          View Opportunities in <u>
            <div class="country-select" v-if="countryValue" onmouseover="" style="cursor: pointer;  text-decoration: underline;">
              <multiselect v-model="countryValue"
                           :options="rawCountryData"
                           :searchable="false"
                           track-by="name"
                           label="name"
                           :hide-selected="false"
...
<script>
  ...
  methods: {
  getImage() {
  axios.get('https://app.panxpan.com/api/countries/' + this.countryValue.code2+'/imageUrl')
  .then(image=> {
  this.marketingImage= image.data.data;
  this.countryLogo = 'https://pxpgp.blob.core.windows.net/marketing-site/countries-flag/'+this.countryValue.code2.toLowerCase().trim()+'.svg';
  }).catch(e => {
  })
  }
  },
  ...
  created () {
  if (this.countrySelectedData) {
  var code2 = this.countrySelectedData
  this.countryValue = this.rawCountryData.find(function (country) {
  console.log("Code 2 value"+code2);
  ...
  this.marketingImage = image.data.data;
  this.countryLogo = 'https://pxpgp.blob.core.windows.net/marketing-site/countries-flag/'+this.countryValue.code2.toLowerCase().trim()+'.svg';
  console.log("countrylogo"+this.countryLogo);
...
<style>
  ...
  .country-select .multiselect__input, .country-select .multiselect__single
  {
  font-size: 20px;
  }
  #bgImage{
