<template>
  <header>
    <div class="pxp-navbar container" v-bind:class="{ whiteNav: pageScroll }">

      <b-navbar toggleable="md" fixed="top">

        <b-nav-toggle target="nav_collapse"></b-nav-toggle>

        <b-navbar-brand href="../" class="pxp-logo">
          <img src="../assets/images/logo-main.png" class="black-logo">
            <img src="../assets/images/logo-main-white.png" class="white-logo">
        </b-navbar-brand>
        <b-collapse is-nav="" id="nav_collapse">
          <b-navbar-nav is-nav-bar="" class="ml-auto">
            <b-nav-item-dropdown text="Manufacturers" left="">
              <b-dropdown-item href="../manufacturers/how-it-works">How It Works</b-dropdown-item>
              <b-dropdown-item href="../manufacturers/building-trust">Building Trust</b-dropdown-item>
              <b-dropdown-item v-bind:href="getUrl('signup')">Signup</b-dropdown-item>
            </b-nav-item-dropdown>
            <b-nav-item-dropdown text="Local Partners" left="">
              <b-dropdown-item href="../local-partners/how-it-works">How It Works</b-dropdown-item>
              <b-dropdown-item href="../local-partners/building-trust">Building Trust</b-dropdown-item>
              <b-dropdown-item href="../find/search-products">
                View Opportunities in {{countryValue.name}}
              </b-dropdown-item>
              <b-dropdown-item v-bind:href="getUrl('signup')">Signup</b-dropdown-item>
            </b-nav-item-dropdown>
            <b-nav-item href="//knowledge.panxpan.com" >Knowledge</b-nav-item>
          </b-navbar-nav>
          <b-navbar-nav is-nav-bar="" class="ml-auto">
            <b-nav-item-dropdown left="" :text="'Hi ' + user.userName" v-if="user">
              <b-dropdown-item v-bind:href="getLogoutUrl()">Launch application</b-dropdown-item>
              <b-dropdown-item v-bind:href="getLogoutUrl()">Logout</b-dropdown-item>
            </b-nav-item-dropdown>
            <b-nav-item v-if="!user" v-bind:href="getUrl('login')" class="navbar-btn">Login</b-nav-item>
            <b-nav-item v-if="!user" v-bind:href="getUrl('signup')" class="navbar-btn transparent white">Sign Up</b-nav-item>
          </b-navbar-nav>
        </b-collapse>
      </b-navbar>
    </div>
...
<script>
  ...
  getUrl(method){
  return process.env.appUrl+method;
  },
  scrollMethod () {
  window.addEventListener('scroll', () => {
  var top = window.pageYOffset || document.documentElement.scrollTop
  if (top > 80 || this.message) {
  this.pageScroll = true
  } else {
  ...
  if (this.token.data != undefined && this.token.data.access_token) {
  ...
  ifrm.setAttribute('src', '//apppxpdev.panxpan.com/frame.html')
...
let val = JSON.parse(localStorage.getItem('ipstack'));
if(val != null)
{
console.log('localsorage');
console.log(val);
function findCountry (country) {
  return country.code2 === val.data.country_code
  }
...
console.log('from api');
  axios.get('https://api.ipstack.com/' + response.data.ip + '?access_key=18f351f4672cccbcdbdd27202976e1f1')
  .then(data => {
  console.log(data);
  localStorage.setItem('ipstack', JSON.stringify(data));
  function findCountry (country) {
  return country.code2 === data.data.country_code
  }
  this.countryValue = this.rawCountryData.find(findCountry)
  })
  .catch(e => {
